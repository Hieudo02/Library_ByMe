...
double ui, ui_p = 0;
int ui_above_limit = 500.0;
int ui_under_limit = 0.0;

// khai biến output
double u;
int u_above_limit = 500.0;
...
void PID_CAL(TimerHandle_t xTimer) {
  myTIM_STEP.ChayStep(u);
  
  /******ENCODER*****/
  pulse = myEncoder.DocXungEncoder();
...
void setup() {
  Serial.begin(115200);
  myEncoder.DatCheDoDocEncoder(mode);
  myTIM_STEP.KhoiTaoStep(dirPin, pulPin, enPin, microStep);
